    ...
    def build(self):
        ...
        if self.config.model == 'MLP':
            ...
            if self.modal=='visual':
                ...
                elif self.config.type=='trans_ib':
                    self.model = SL_module_ib(input_dim=1024, depth=5, heads=8, mlp_dim=3072, dropout_ratio=0.5)
                else:
            ...
            if self.modal=='audio':
                if self.config.type=='base':
                    ...
                    self.model = SL_module_ib(input_dim=128, depth=5, heads=8, mlp_dim=3072, dropout_ratio=0.5)
                else:
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for _ in tqdm(range(num_batches)):
                ...
                #print(input_feature)
                    ...
                    total_loss = prediction_loss + beta * kl_loss
                    total_loss.backward()
                    loss_v_history.append(prediction_loss.detach().item())
                    kl_loss_history.append(kl_loss.detach().item())
                    loss_history.append(total_loss.detach().item())
                    self.optimizer.step()
            ...
            elif self.config.type=="ib" or self.config.type=="cib" or self.config.type=="eib" or self.config.type=="lib" or self.config.type == "trans_ib":
