...
class MrHiSumDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        d['features'] = torch.Tensor(np.array(self.features_data[video_name + '/features']))
        d['gtscore'] = torch.Tensor(np.array(self.video_data[video_name + '/gtscore']))
        d['audio'] = torch.Tensor(np.array(self.features_data[video_name + '/audio']))
...
class BatchCollator(object):
    def __call__(self, batch):
        ...
        video_name, features, gtscore, audio= [],[],[],[]
        ...
        batch_data = {'video_name' : video_name, 'features' : frame_feat, 'gtscore':gtscore, 'mask':mask, 'audio':audio}
        # batch_data = {'video_name' : video_name, 'features' : frame_feat, 'gtscore':gtscore, 'mask':mask, \
