#!/bin/bash
#python main_dy_cmib.py --train True  --batch_size 80 --modal multi --device 0 --lr 0.05 --type VsameA_cmib_SMI --vbeta 1e-01 --abeta 1e-01 --mbeta 1e-01 --epochs 50 --tag mr_VsameA_cmib_SMI --path dataset/mr_hisum_split.json
time_tag="04282100"
type="VsameA_cmib_SMI"
modal="multi"
#1e-06
# 定義所有 dataset 路徑
datasets=(
...
do
    ...
    do
        ...
        python main_dy_cmib.py --train True  --batch_size 40 --modal ${modal} --device 2 --lr 0.05 --type ${type} --vbeta ${value} --abeta ${value} --mbeta ${value} --epochs ${i} --tag ${cate}_${type} --path ${path} --SMI True
    done
done
