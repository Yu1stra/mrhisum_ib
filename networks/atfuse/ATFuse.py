...
class MHCABlock(nn.Module):
    ...
    def forward(self, q, k, v, minus=True):
        ...
        b, c, = q.size(0), q.size(1)
        h = w = int(math.sqrt(int(q.size(2))))
        #print(b,c,h,w)
...
class UpScale(nn.Module):
    def __init__(self, is_feature_sum, embed_in, embed_dim ):
        #print("embed_in=",embed_in)
        ...
        if is_feature_sum:
            self.conv11_headSum = nn.Conv1d(embed_in, embed_in, kernel_size=3,
                                            stride=1, padding=1, bias=True)
        else:
            self.conv11_head = nn.Conv1d(embed_in * 2, embed_in, kernel_size=3,
                                         stride=1, padding=1, bias=True)
        self.conv12 = nn.Conv1d(embed_in, embed_in * 2, kernel_size=3,
                                stride=1, padding=1, bias=True)
        self.ps12 = nn.PixelShuffle(1)
        #self.conv11_tail = nn.Conv1d(embed_in // 2, embed_in // 2, kernel_size=3,stride=1, padding=1, bias=True)
        self.conv11_tail = nn.Conv1d(embed_in *2, embed_in  , kernel_size=3, stride=1, padding=1, bias=True)

    def forward(self, x, x_res):
        x = x.flatten(2)
        x11 = x  # B, C, H, W
        #print(f"x,x_res={x.shape,x_res.shape}")
        if self.is_feature_sum:
            #print("in is_feature_sum")
            x = x + x_res  # B, C, H, W
            #print("x+x_res=",x.shape)
            x = self.conv11_headSum(x)  # B, C, H, W
            #print("after headsum=",x.shape)
            #print("--------------------------------------------------------")
        else:
            #print("in else")
