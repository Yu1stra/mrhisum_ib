...
from model.utils.clip_level_processing import (batch_process_clip_level, map_batch_clip_scores_to_frames,
                                           process_clip_level_features, process_clip_level_scores,
                                           map_clip_scores_to_frames)
...
class Solver(object):
    ...
    def build(self):
        ...
        if self.config.type=='base':
            ...
            self.model = SL_module_IB_tran(input_dim=1024, depth=5, heads=8, mlp_dim=3072, dropout_ratio=0.5, bottleneck_dim=256)
    ...
    def train(self):
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for _ in tqdm(range(num_batches)):
                ...
                change_points = data['change_points']  # 获取change points
                
                # 处理为clip级别的数据
                clip_data = batch_process_clip_level(visual, gtscore, audio, change_points)
                clip_features = clip_data['padded_clips']  # [total_clips, max_clip_len, feature_dim]
                clip_scores = clip_data['clip_scores']      # [total_clips]
                clip_mask = clip_data['clip_mask']          # [total_clips, max_clip_len]
                batch_indices = clip_data['batch_indices']  # [total_clips]
                
                # 打印调试信息，仅在第一个epoch的第一个batch显示
                if epoch_i == 0 and _ == 0:
                    print("\n======= Clip-level Processing Debug Info =======")
                    print(f"Original frame features shape: {visual.shape}")
                    print(f"Original gtscore shape: {gtscore.shape}")
                    print(f"Clip features shape: {clip_features.shape}")
                    print(f"Clip scores shape: {clip_scores.shape}")
                    print(f"Clip mask shape: {clip_mask.shape}")
                    print(f"Number of clips: {len(clip_scores)}")
                    print(f"Number of batches: {len(data['change_points'])}")
                    print(f"First batch clips count: {len(data['change_points'][0])}")
                    print("============================================\n")
                
                #一般---------------------------
                if self.config.type=="base":
                    # 使用clip级别的特征进行预测
                    clip_pred_scores, weights = model(clip_features, clip_mask)
                    
                    # 模型输出是每个帧的得分，需要汇总为每个clip的得分
                    # 使用有效帧(非填充帧)的平均值作为clip得分
                    clip_mask_sum = clip_mask.sum(dim=1)  # [total_clips]
                    clip_pred_aggregated = (clip_pred_scores * clip_mask).sum(dim=1) / clip_mask_sum  # [total_clips]
                    
                    # 计算损失
                    loss = self.criterion(clip_pred_aggregated, clip_scores).mean()
                    loss.backward()
                ...
                # **调用 SL_module**
                    ...
                    # 使用clip级别的特征进行预测
                    clip_pred_scores, kl_loss = model(clip_features, clip_mask)
                    ...
                    # 计算预测损失 (主任务损失)
                    prediction_loss = self.criterion(clip_pred_aggregated, clip_scores).mean()
                    ...
                    if torch.isnan(kl_loss) or torch.isinf(kl_loss):
                        ...
                        kl_loss = torch.tensor(0.0, device=clip_pred_aggregated.device)
...
#final_f_score, final_map50, final_map15, final_loss, final_precision, final_prediction_loss_v, final_prediction_loss_a, final_kl_loss
    ...
    def evaluate(self, dataloader=None):
        ...
        for data in dataloader:
            ...
            clip_mask = clip_data['clip_mask']          # [total_clips, max_clip_len]
            batch_indices = clip_data['batch_indices']  # [total_clips]
            clip_lengths = clip_data['clip_lengths']     # [total_clips]
            
            # Debug information for the first sample in evaluation
            if i == 0 and self.verbose:
                print("\n======= Evaluation Clip-level Processing Debug Info =======")
                print(f"Original frame features shape: {visual.shape}")
                print(f"Original gtscore shape: {gtscore.shape}")
                print(f"Clip features shape: {clip_features.shape}")
                print(f"Clip scores shape: {clip_scores.shape}")
                print(f"Clip mask shape: {clip_mask.shape}")
                print(f"Number of clips: {len(clip_scores)}")
                print(f"Change points in first sample: {change_points[0]}")
                print("===================================================\n")
            ...
            if self.config.type=="base":
                ...
                with torch.no_grad():
                    ...
                    # 将clip级别的预测映射回帧级别，以便与原始评估指标匹配
                    # 使用聚合后的clip分数映射回帧级别
                    score = map_batch_clip_scores_to_frames(clip_pred_aggregated, change_points, batch_indices, n_frames)
            ...
            # Calculate precision - since we've mapped back to frame level, we can use the original mask
            frame_mask = data['mask'].to(cuda_device)
            frame_level_score = score[0] if score.dim() > 1 else score  # Ensure we're working with a 1D tensor
            frame_level_gtscore = gtscore[0] if gtscore.dim() > 1 else gtscore
            
            predictions = (frame_level_score > 0.5).float()  # Example threshold, modify as needed
            true_positives = (predictions[frame_mask] * frame_level_gtscore[frame_mask]).sum().detach().item()
            predicted_positives = predictions[frame_mask].sum().detach().item()
            precision = true_positives / (predicted_positives + 1e-7)  # Avoid division by zero
            precision_history.append(precision)
        
            # Summarization metric
            # Since we're now working with frame-level scores mapped back from clip-level predictions
            # We can use the same summarization process as before
            frame_level_score = score.squeeze().cpu() if score.dim() > 1 else score.cpu()
            gt_summary = data['gt_summary'][0]
            cps = data['change_points'][0]
            n_frames = data['n_frames']
            nfps = data['n_frame_per_seg'][0].tolist()
            picks = data['picks'][0].numpy()
            #print("score",len(frame_level_score), "cps",len(cps), "nframe",len(n_frames), "nfps",len(nfps), "picks",len(picks))
            machine_summary = generate_summary(frame_level_score, cps, n_frames, nfps, picks)
            ...
            # Highlight Detection Metric - adapt to work with frame-level scores mapped from clip-level
            ...
            # frame_level_score has already been created for the summary generation
            # Use it for the highlight detection metrics as well
            highlight_seg_machine_score = generate_mrhisum_seg_scores(frame_level_score, uniform_clip=5)
