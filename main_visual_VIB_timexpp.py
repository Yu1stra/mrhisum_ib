...
# TimeX++的导入
import sys
sys.path.append('/home/jay/TimeXplusplus')
...
class Solver(object):
    ...
    def build(self):
        ...
        for data in self.train_loader:
            ...
            if features.size(1) > max_seq_length:
                max_seq_length = features.size(1)
        ...
        if self.config.type=='base':
            ...
            # 计算视频特征的均值和标准差，用于掩码令牌
            all_features = []
            for data in self.train_loader:
                all_features.append(data['features'])
            all_features = torch.cat(all_features, dim=0)
            mu = all_features.mean(dim=1)
            std = all_features.std(unbiased=True, dim=1)
            
            self.model = TimeXModel(
                d_inp=1024,  # 视觉特征维度
                max_len=max_seq_length,
                n_classes=1,  # 回归任务
                n_prototypes=30,  # 原型数量
                gsat_r=0.5,  # GSAT损失参数
                transformer_args=timex_transformer_args,
            ...
            # 定义TimeX++的特殊损失函数
            self.clf_criterion = nn.MSELoss(reduction='mean').to(self.device)
            self.sim_criterion_label = LabelConsistencyLoss()
            self.sim_criterion_cons = EmbedConsistencyLoss(normalize_distance=False)
            self.sim_criterion = [self.sim_criterion_cons, self.sim_criterion_label]
        
        self.model.to(cuda_device)
        
        # 优化器设置
        if self.config.type == 'timex':
            self.optimizer = optim.AdamW(self.model.parameters(), lr=1e-3, weight_decay=0.001)
        else:
            # VIB模型的优化器设置
            ib_params = []
            other_params = []
            ...
            ib_lr = self.config.lr * 0.1
            print(f"使用学习率: 一般参数 {self.config.lr}, VIB参数 {ib_lr}")
            
            self.optimizer = optim.SGD([
                {'params': ib_params, 'lr': ib_lr, 'weight_decay': self.config.l2_reg * 0.1},  # 变分参数
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for _ in tqdm(range(num_batches)):
                ...
                #TimeX++-----------------------------------------------------
                    ...
                    # 创建注意力掩码，用于处理变长序列
                    attn_mask = transform_to_attn_mask(mask)
                    attn_mask = attn_mask.to(cuda_device)
                    
                    # TimeX++前向传播
                    output_dict = model(visual, times=None, mask=attn_mask)
                    
                    # 提取预测分数和其他输出
                    pred_scores = output_dict['y_pred'].squeeze(-1)  # [B, T]
                    pred_scores = pred_scores.unsqueeze(-1)  # [B, T, 1]
                    
                    # 计算MSE回归损失
                    prediction_loss = self.criterion(pred_scores[mask], gtscore[mask]).mean()
                    
                    # 提取TimeX++的特殊损失
                    gsat_loss = output_dict.get('gsat_loss', torch.tensor(0.0, device=cuda_device))
                    connect_loss = output_dict.get('connected_loss', torch.tensor(0.0, device=cuda_device))
                    
                    # 计算一致性损失（如果可用）
                    if 'h_cont' in output_dict and 'h_cont_mix' in output_dict:
                        h_cont = output_dict['h_cont']
                        h_cont_mix = output_dict['h_cont_mix']
                        y_cont = output_dict['y_cont']
                        y_cont_mix = output_dict['y_cont_mix']
                        
                        # 使用TimeX++的一致性损失函数
                        consistency_loss_embed = self.sim_criterion[0](h_cont, h_cont_mix)
                        consistency_loss_label = self.sim_criterion[1](y_cont, y_cont_mix)
                        consistency_loss = consistency_loss_embed + consistency_loss_label
                    else:
                        consistency_loss = torch.tensor(0.0, device=cuda_device)
                    
                    # 组合所有损失，权重可以根据实际情况调整
                    total_loss = prediction_loss + 0.1 * gsat_loss + 0.05 * connect_loss + 0.01 * consistency_loss
                    ...
                    loss_a_history.append(prediction_loss.detach().item())  # 使用预测损失作为主任务损失
                    
                    # 将GSAT损失视为KL损失（对于记录目的）
                    kl_loss_history.append(gsat_loss.detach().item() if hasattr(gsat_loss, 'item') else 0.0)
                ...
                #print(input_feature)
                    ...
                    loss_a_history.append(pred_loss_val)
    ...
    def evaluate(self, dataloader=None):
        ...
        for data in dataloader:
            ...
            #TimeX++----------------------------------------------
                ...
                with torch.no_grad():
                    # TimeX++模型前向传播
                    output_dict = model(input_feature, times=None, mask=attn_mask)
                ...
                # 记录损失
                loss_history.append(prediction_loss.detach().cpu().item())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type',type = str, default='ib', help='base,ib,cib,eib,lib,timex')#支持TimeX++模型
